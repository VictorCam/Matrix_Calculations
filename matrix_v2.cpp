...
int main() {
  ...
  while(c1 != NULL) {
    mat1_arr[n1] = stoi(c1);
    c1 = strtok(NULL, "\t\n ");
    n1++; //we count the number of values that exist in matrix 1
  }
...
//  cout << "c1: " << c1 << " :c1" << endl;
  ...
  while(c2 != NULL) {
    mat2_arr[n2] = stoi(c2);
    c2 = strtok(NULL, "\t\n ");
    n2++; //we count the number of values that exist in matrix 2
  }
...
//STARTING RUNTIME HERE
  ...
  if((cols1+rows2) == (cols2+rows1)) { //for example of matrix is 25 x 25 it should be this stoping condition
    ...
    while(true) {
      if(inc6 == stop){ //will eventually reach break once it reaches it suffice number of executions
        cout << endl << "quiting program";
        break;
      }
      ...
      if (inc1 == rows1) {
        ...
        if(inc5 == cols2) { //here we print the new number without a new line or tab.
          ...
          //cout << sum << " ";
          sum -= sum; //reset sum
          inc1 -= inc1; //restart the counter again
